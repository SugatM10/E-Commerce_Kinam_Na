...
class AddToCartView(EcomMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        recommend_products = recommend(product_obj.title)
        # GEt product detail = get_detail(recommend_prodcts)
        new_recommend_products = []
        for p in recommend_products:
            try:
                r_p = Product.objects.filter(Q(title=p))
                new_recommend_products.append(r_p[0])
            except:
                continue
        # print(recommend_products)
        # print(new_recommend_products)
        context['r_products'] = new_recommend_products  # product detail
        return context
